...
class Environment:
    ...
    def step(self, action):
        ...
        self.PB_cache()                                                     # 缓存策略基于上一个大时隙的流行度
    ...
    def PB_cache(self):
        ...
        popularity = np.zeros(self.task_num)        # 记录上一个大时隙各类任务的请求数
        remaining_capacity = self.server_c          # 剩余缓存容量
        for user_index in range(self.user_num):
            for slot_index in range(1, self.s_slot_num+1):
                if self.user_req[user_index][slot_index] != 0:
                    popularity[self.user_req[user_index][slot_index] - 1] += 1
        sorted_popularity = np.argsort(popularity)[::-1]
        for i in range(self.task_num):
            # 如果缓存已满，则跳出循环
            if remaining_capacity <= 0:
                break
            # 按流行度顺序缓存任务请求
            task_idx = sorted_popularity[i]
            # 如果缓存剩余容量不足以缓存选定任务，则跳出循环
